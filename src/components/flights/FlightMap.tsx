...
function CountryFlag({code}:{code:string}){
  if(!/^[A-Z]{2}$/.test(code))return <span aria-hidden="true">🌐</span>;
  // Local SVGs avoid platform-dependent flag emoji rendering on Windows.
  // eslint-disable-next-line @next/next/no-img-element
  return <img className="country-flag" src={`/flags/${code.toLowerCase()}.svg`} alt={`${code} flag`}/>;
}
...
export default function FlightMap({ flights }: Props) {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      mapRef.current=map;
      const info=new maps.InfoWindow({ maxWidth:256, disableAutoPan:true }),bounds=new maps.LatLngBounds();
...
function SummaryBlock({value,label,children}:{value:string,label:string,children:React.ReactNode}){return <article className="fr24-summary"><div className="fr24-summary-top"><div><strong key={`${value}-${label}`} className="flight-value-change">{value}</strong><span key={label} className="flight-value-change">{label}</span></div></div><div className="fr24-equivalents">{children}</div></article>}
function Ranking({title,flightRows,distanceRows,total,unit,airlines=false,aircraft=false,countries=false,airports=false,routeLabels=false}:{title:string,flightRows:[string,number][],distanceRows:[string,number][],total:string,unit:'km'|'mi',airlines?:boolean,aircraft?:boolean,countries?:boolean,airports?:boolean,routeLabels?:boolean}){const[metric,setMetric]=useState<'flights'|'distance'>('flights'),rows=(metric==='flights'?flightRows:distanceRows).slice(0,9),max=rows[0]?.[1]||1,[totalNumber,...totalWords]=total.split(' ');return <article className={`fr24-panel ranking-panel${routeLabels?' route-ranking':''}`}><PanelHeader title={title} metric={metric} setMetric={setMetric} stacked/><div className="rank-list">{rows.map(([name,value])=><div key={name}>{airlines?<AirlineLabel code={name}/>:countries?<span title={name}><CountryFlag code={name}/> {name}</span>:airports?<span title={AIRPORTS[name]?.name}><CountryFlag code={AIRPORTS[name]?.country||''}/> {name}</span>:routeLabels?<RouteLabel route={name}/>:aircraft?<span title={name}>{aircraftAbbreviation(name)}</span>:<span>{name}</span>}<i><b style={{width:`${value/max*100}%`}}/></i><strong key={`${metric}-${unit}-${value}`} className="flight-value-change">{metric==='distance'?`${formatDistance(value,unit)} ${unit}`:Math.round(value)}</strong></div>)}</div><footer><strong>{totalNumber}</strong><span>{totalWords.join(' ')}</span></footer></article>}
function RouteLabel({route}:{route:string}){const[from,to]=route.split('–');return <span className="route-label" title={route}><CountryFlag code={AIRPORTS[from]?.country||''}/><span>{route}</span><CountryFlag code={AIRPORTS[to]?.country||''}/></span>}
...
function ExtremeFlightRanking({title,flights,unit}:{title:string,flights:FlightRecord[],unit:'km'|'mi'}){const unique=[...new Map(flights.map(f=>[[f.from,f.to].sort().join('–'),f])).values()],rows=unique.slice(0,9),max=Math.max(1,...unique.map(distanceKm));return <article className="fr24-panel ranking-panel extreme-ranking"><RankingTitle title={title}/><div className="extreme-list">{rows.map((flight,index)=>{const from=AIRPORTS[flight.from],to=AIRPORTS[flight.to],distance=distanceKm(flight);return <div key={`${flight.from}-${flight.to}-${index}`} title={`${from.city} to ${to.city} · ${formatDuration(estimatedDuration(flight))}`}><span className="route-label"><CountryFlag code={from.country}/><span>{flight.from}–{flight.to}</span><CountryFlag code={to.country}/></span><i><b style={{width:barWidth(distance,max)}}/></i><strong key={`${unit}-${distance}`} className="flight-value-change">{formatDistance(distance,unit)} {unit}</strong></div>})}</div><footer><strong>{unique.length}</strong><span>unique routes</span></footer></article>}
function RankingTitle({title}:{title:string}){const lines=title==='Top regions & countries'?['Top regions','& countries']:[title.split(' ')[0],title.split(' ').slice(1).join(' ')];return <h3 className="ranking-title"><span>{lines[0]}</span><span>{lines[1]}</span></h3>}
...
function MiniChart({title,flightRows,distanceRows,unit,metric,setMetric}:{title:string,flightRows:[string,number][],distanceRows:[string,number][],unit:'km'|'mi',metric:'flights'|'distance',setMetric:(metric:'flights'|'distance')=>void}){
  ...
  return <article className="fr24-panel fr24-chart"><PanelHeader title={title} metric={metric} setMetric={setMetric}/><div className="mini-bars">{rows.map(([label,value])=>{const shown=metric==='distance'?formatCompactDistance(value,unit):String(Math.round(value));return <div key={label} className="bar-cell"><strong key={`${metric}-${unit}-${shown}`} className="flight-value-change">{shown}</strong><div className="bar-track"><span style={{height:`${Math.max(3,value/max*100)}%`}}/></div><small>{label}</small></div>})}</div></article>;
}
